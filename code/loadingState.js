...
Mario.LoadingState.prototype.CheckForChange = function (context) {
    if (this.ImagesLoaded) {
        ...
        // Initialize global Mario character with detailed debugging
        console.log('Checking Mario.Character availability:', typeof Mario.Character);
        if (typeof Mario.Character !== 'undefined') {
            try {
                Mario.MarioCharacter = new Mario.Character();
                console.log('Mario character created successfully:', Mario.MarioCharacter);

                if (Enjine.Resources.Images["smallMario"]) {
                    Mario.MarioCharacter.Image = Enjine.Resources.Images["smallMario"];
                    console.log('Mario character image set');
                } else {
                    console.warn('smallMario image not available');
                }
                console.log('Mario character initialized successfully');
            } catch (error) {
                console.error('Failed to create Mario character:', error);
            }
        } else {
            console.error('Mario.Character constructor not available!');
        }

        // Initialize treasure chest system
        if (typeof Mario.TreasureChest !== 'undefined') {
